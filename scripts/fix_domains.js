        ...
        if(file.endsWith("en.json") && parsed.requirements) {
                ...
                parsed.rewards.forEach(elem => {
                  elem.details.forEach(detail => {
                    ...
                    detail.adventureExperience = Number.parseInt(detail.adventureExperience)
                    detail.companionshipExperience = Number.parseInt(detail.companionshipExperience)
                    detail.mora = Number.parseInt(detail.mora)
                    if(detail.drops) {
                        detail.drops = detail.drops.map(entry => ({name: entry.name, drop_min: Number.parseInt(entry.rate.split('-')[0]), drop_max: Number.parseInt(entry.rate.split('-')[1])}))

                    }
                    if(detail.items) {
                        detail.items = detail.items.map(entry => ({name: entry.name, drop_min: Number.parseInt(entry.rate.split('-')[0]), drop_max: Number.parseInt(entry.rate.split('-')[1])}))
